                    ...
                    <div class="col-md-8">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <a class="btn btn-warning btn-lg" data-mdb-ripple-color="dark"
                                    href="{{ url('/') }}/message/{{ $news->id }}/edit">แก้ไข</a>
                                <form action="{{ url('/') }}/message/{{ $news->id }}" method="POST"
                                    onsubmit="return validate();" style="display:inline">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <button class="btn btn-danger btn-lg" data-mdb-ripple-color="dark"
                                        type="submit">ลบ</button>
                                </form>
                                <script>
                                    function validate() {
                                        var wantToDelete = confirm('คุณต้องการลบข้อมูลข่าวสารนี้ ใช่หรือไม่?');
                                        if (wantToDelete) {
                                            return true;
                                        } else {
                                            return false;
